...
class NapDataArray {
    ...
    constructor(n) { // NapData[]
        ...
        console.log(`[NapDataArray] Constructor called with ${n.length} bytes`);
        try {
            this.bitVals = this.getBitValsSigned(n);//pow(2, (n.length * this.bitsPerByte) - parseInt(this.firstBitSign)); // float
            console.log(`[NapDataArray] bitVals calculated: ${this.bitVals}`);
        } catch (e) {
            console.error(`[NapDataArray] Error in constructor:`, e);
            throw e;
        }
    }
    ...
    getBitValsSigned(n) {
        const returns = Math.pow(2, (n.length * this.bitsPerByte) - parseInt(this.firstBitSign))
    ...
    binaryConv(n, loc) { // NapData, int
        ...
        console.log(`[binaryConv] loc=${loc}, bitsPerByte=${this.bitsPerByte}, result="${returns}"`);
        return returns;
    }
...
class NapVector extends NapDataArray {
    ...
    constructor(n) { // NapData[]
		super(n);
        console.log(`[NapVector] Creating vector with ${n.length} bytes: ${n.map(d => d.hex).join(' ')}`);
        try {
            this.x = this.getCoordFromBytes(n, "x"); // float
            this.y = this.getCoordFromBytes(n, "y"); // float
            //this.z = this.getCoordFromBytes(n, "z"); // float
            console.log(`[NapVector] Successfully decoded: (${this.x}, ${this.y})`);
        } catch (e) {
            console.error(`[NapVector] Error decoding coordinates:`, e);
            throw e;
        }
    }
...
*/  
    getSingleByteVal(n, axis) { // NapData, string
        ...
        console.log(`[getSingleByteVal] ${axis}: ${returns} (from ${n.length} bytes)`);
        return returns;
    }
    ...
    getCoordFromBytes(n, axis) { // NapData[], string
        ...
        // Verbose Coordinate Decoder
        const bytes = n.map(d => d.hex).join(' ');
        console.log(`[DECODE] ${axis.toUpperCase()} bytes: ${bytes} → Value: ${finalReturns}`);
...
class NapCmd {
    ...
    constructor(_cmd, _index) { // string, int
        ...
        switch(this.opcode.id) {
            ...
            //~ ~ ~ POINTS ~ ~ ~
            case("POINT SET ABS"):
                if (this.data.length > 0) this.setPoints(this.data, true);
                break;
            case("POINT SET REL"):
                if (this.data.length > 0) this.setPoints(this.data, true);
                break;
            case("POINT ABS"):
                if (this.data.length > 0) this.setPoints(this.data, false);
                break;
            case("POINT REL"):
                if (this.data.length > 0) this.setPoints(this.data, false);
                break;
    ...
    setPoints(data, setCursor) {
        // Handle case where data is a boolean (legacy calls)
        ...
        for (let i = 0; i < totalBytes; i += pointByteLength) {
            ...
            const rawX = data.slice(i, i + bytesPerCoord).map(d => d.c.charCodeAt(0));
    ...
    setDomain() {
		...
		*/
    	...
    	// Domain Byte Decoder Debugging
    	console.log(`[DOMAIN] Raw domain byte: 0x${this.data[0].hex.toUpperCase()}`);
    	console.log(`[DOMAIN] Binary: ${domainByte}`);
    	console.log(`[DOMAIN] Point bytes (bits 2,3,4): ${domainPointBytes}`);
    	console.log(`[DOMAIN] Single bytes (bits 5,6): ${domainSingleBytes}`);
    	...
    	console.log("Domain byte binary: " + domainByte);
    	console.log("Domain point bytes (bits 2,3,4): " + domainPointBytes);
    	console.log("Domain single bytes (bits 5,6): " + domainSingleBytes);
...
class NapDecoder {
    ...
    parseCommands(input) {
        ...
        for (let i=0; i<returns.length; i++) {
            ...
            const cmd = returns[i];
            console.log(`[CMD] Opcode: 0x${cmd.opcode.hex.toUpperCase()} → ${cmd.opcode.id}`);
        }    
...
class NapEncoder {
    ...
    getBitValsSigned(n) {
        return pow(2, (n.length * this.bitsPerByte) - parseInt(this.firstBitSign))
    }
...
if (typeof window !== 'undefined') {
    ...
    window.NapCmd = NapCmd;
    window.NapEncoder = NapEncoder;
    window.NapInputWrapper = NapInputWrapper;
    window.Vector2 = Vector2;
    window.Vector3 = Vector3;
    window.NapChar = NapChar;
    window.NapOpcode = NapOpcode;
    window.NapData = NapData;
